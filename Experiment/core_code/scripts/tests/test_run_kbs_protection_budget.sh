...
export CHECKPOINT="$scratch/checkpoint space.pt" ORACLE_OUTPUT_DIR="$scratch/oracle source"
export REPAIR_OUTPUT_DIR="$scratch/pilot source" PROTECTION_BUDGET_OUTPUT_DIR="$scratch/new output"
export GPU_ID=2 SEEDS=9 ORACLE_SEEDS=7 REPAIR_SEEDS=8 DEVICE=cpu STEPS=4 OUTPUT_DIR=ignored
unset PROTECTION_BUDGET_SEEDS
bash "$script_dir/run_kbs_protection_budget.sh" run --seeds 0,1
python3 - "$CALLS" <<'PY'
import json, os, sys
...
for r in rows:
    ...
    for flag, env in [('--study-dir', 'STUDY_DIR'), ('--cache-dir', 'CACHE_DIR'),
                      ('--pilot-dir', 'REPAIR_OUTPUT_DIR'), ('--oracle-dir', 'ORACLE_OUTPUT_DIR'),
                      ('--output-dir', 'PROTECTION_BUDGET_OUTPUT_DIR'), ('--checkpoint', 'CHECKPOINT')]:
...
for stage in select train; do
    ...
    : > "$CALLS"
    set +e
    bash "$script_dir/run_kbs_protection_budget.sh" run
    status=$?
    set -e
    [[ "$status" == 43 ]]
...
unset FAIL_STAGE
export PROTECTION_BUDGET_SEEDS=1,2
: > "$CALLS"
for mode in plan preflight select train evaluate; do
    bash "$script_dir/run_kbs_protection_budget.sh" "$mode"
done
python3 - "$CALLS" <<'PY'
import json, sys
...
PY
echo 'Protection budget launcher tests passed'
