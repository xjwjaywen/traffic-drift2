...
Experiments on CESNET-TLS-Year22 (178 classes, 12 months) and CESNET-QUIC22 (102 classes, 4 weeks) show that \ours{} recovers 9 of 12 collapsed classes to usable recall levels using only margin-based selection (no absorber knowledge required), improving collapse-class F1 from 0.028 to 0.262 (9$\times$) with only 1{,}000 labeled samples from the target period.
The overall macro-F1 improves from 0.629 to 0.686 (+5.7\%), while stable-class F1 decreases only marginally (0.903$\to$0.895).
