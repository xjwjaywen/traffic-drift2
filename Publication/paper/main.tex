...
Experiments on CESNET-TLS-Year22 (178 classes, 12 months) and CESNET-QUIC22 (102 classes, 4 weeks) demonstrate that \ours{} recovers 7 out of 12 collapsed classes, improving collapse-class F1 from 0.028 to 0.426 while maintaining stable-class performance (F1: 0.903$\to$0.891). With only 1{,}000 labeled target samples ($<$0.6\% of the test set), \ours{} achieves an overall macro-F1 of 0.683, outperforming the best baseline by 5.4 percentage points.
\end{abstract}
...
Experiments on CESNET-TLS-Year22 (178 classes, 12 months) and CESNET-QUIC22 (102 classes, 4 weeks) show that \ours{} recovers 7 of 12 collapsed classes to usable recall levels, improving collapse-class F1 from 0.028 to 0.426 (+15$\times$) with only 1{,}000 labeled samples from the target period.
Three classes achieve recall above 0.62, with one reaching 0.91.
The overall macro-F1 improves from 0.629 to 0.683 (+5.4\%), while stable-class F1 decreases only marginally (0.903$\to$0.891).
...
As we show in Section~\ref{sec:tta_eval}, they are ineffective for traffic classification, where drift manifests as systematic class confusion rather than global distribution shift.
...
\midrule
\textbf{\ours{}} & \textbf{.559} & \textbf{.298} & \textbf{.426} & \textbf{.744} & \textbf{.696} & \textbf{.683} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Baselines.}
We compare against: (1)~Static (no adaptation), (2)~BN-Adapt~\cite{schneider2020bn}, (3)~Tent~\cite{wang2021tent}, (4)~EATA~\cite{niu2022eata}, (5)~CoTTA~\cite{wang2022cotta}, (6)~SAR~\cite{niu2023sar}, (7)~NOTE~\cite{gong2022note}, and (8)~CAPS (our prototype-based method).
...
Budget $B=1000$, replay mode \texttt{all} with $k=5$ per class, target repeat factor 2, distillation weight $\lambda_{\text{KD}}=0.5$, temperature $T=2.0$, fine-tuning learning rate $10^{-3}$, 30 epochs.
...
\midrule
\textbf{\ours{}} & \textbf{0.683} & \textbf{0.426} & 0.891 & \textbf{5} \\
\bottomrule
\end{tabular}
\end{table}
...
38  & 0.052 & \textbf{0.534} & 0.071 & 0.518 & Substantial \\
163 & 0.015 & \textbf{0.455} & 0.076 & 0.446 & Moderate \\
66  & 0.001 & \textbf{0.355} & 0.002 & 0.270 & Partial \\
174 & 0.000 & 0.274 & 0.000 & 0.230 & Partial \\
\midrule
26  & 0.019 & 0.051 & 0.020 & 0.065 & Failed \\
69  & 0.005 & 0.000 & 0.001 & 0.000 & Failed \\
...
\begin{table}[t]
\centering
\caption{Ablation study at M-2022-12.}
\label{tab:ablation}
\small
\begin{tabular}{@{}l ccc@{}}
\toprule
Configuration & Macro-F1 & Collapse F1 & Stable F1 \\
\midrule
Static (no repair)       & 0.629 & 0.028 & 0.903 \\
Fine-tuning only         & 0.607 & 0.077 & 0.862 \\
Source replay only       & 0.573 & 0.076 & 0.833 \\
Distillation only        & 0.623 & 0.068 & 0.892 \\
FT + Replay (no distill) & 0.604 & 0.116 & 0.849 \\
\midrule
\textbf{\ours{} (full)}  & \textbf{0.683} & \textbf{0.426} & 0.891 \\
\bottomrule
\end{tabular}
\end{table}
...
Key findings:
\begin{itemize}
  \item Fine-tuning alone improves collapse F1 but damages stable classes ($-4.1$\% stable F1), confirming the need for replay and distillation.
  \item Source replay alone reduces collapsed classes (12$\to$9) but severely hurts overall performance ($-5.6$\% macro-F1).
  \item Distillation alone preserves overall performance (0.623 $\approx$ 0.629) while modestly improving collapse F1, confirming its role as a stabilizer.
  \item The full combination achieves both collapse repair \textit{and} overall improvement---the only configuration where macro-F1 exceeds the static baseline.
\end{itemize}

\subsection{Multi-Period Analysis}

% TODO: Add figure showing collapse F1 over M7/M10/M12

\subsection{Generalization to QUIC22}

On CESNET-QUIC22, where only one class collapses (class~71 at W47), \ours{} still improves overall macro-F1 by 3.5\% (0.719$\to$0.748), demonstrating effectiveness even in mild-drift scenarios.

\subsection{Selection Strategy Comparison}

% TODO: Compare absorber_margin vs random vs hybrid_risk

\subsection{Label Efficiency}

% TODO: Budget sweep (200, 500, 1000)
...
% TODO: Analyze which stable classes are affected
...
Our current implementation identifies absorber-victim pairs through confusion analysis on a labeled reference period.
In deployment, operators can detect absorber behavior by monitoring for anomalous increases in per-class prediction frequency, or through periodic sampling of a small labeled validation set.
