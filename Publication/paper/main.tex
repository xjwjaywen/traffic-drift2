...
FT + Replay (no distill)            & 0.640 & 0.350 & 0.910 & 3 \\
\midrule
...
\begin{itemize}
  ...
  \item \textbf{Adding source replay} restores stable-class performance (0.818$\to$0.910) and further improves collapse repair (0.224$\to$0.350). Replay provides training signal from stable classes that prevents head overfitting to the small target sample.
...
\ours{}'s advantage grows with drift severity: at M7 the improvement is modest (+0.06), while at M12 it is dramatic (0.028$\to$0.426).
...
On CESNET-QUIC22, where only one class collapses (class~71 at W47), \ours{} still improves overall macro-F1 by 3.5\% (0.713$\to$0.748 at W46) and accuracy by 9.6\%.
This demonstrates that the method provides benefit even in mild-drift scenarios where class collapse is not yet severe.
...
The ablation shows that without distillation, this degradation is much worse (0.818); distillation limits the trade-off to an acceptable 1.3\%.
