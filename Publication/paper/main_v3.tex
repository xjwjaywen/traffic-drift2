...
Under strict evaluation (BADGE, $B{=}1{,}000$, prototype all-class replay $k{=}5$, 5 seeds), \ours{} raises macro-F1 from $0.507{\pm}0.005$ to $0.565{\pm}0.002$ ($+$11.4\%) and collapse-class F1 from $0.043{\pm}0.010$ to $0.074{\pm}0.016$.
The margin variant achieves $0.560{\pm}0.006$ with comparable collapse F1 ($0.075{\pm}0.017$).
The macro-F1 improvement confirms the framework transfers across protocols and class scales (44 vs.\ 178), though collapse repair remains modest in absolute terms on this dataset.
...
The framework generalizes across drift severity (M7--M12), architectures (CNN/Transformer), and training seeds; an evaluation on QUICEXT-25 shows macro-F1 improvement on a different protocol and class scale, though collapse repair remains limited on that dataset.
