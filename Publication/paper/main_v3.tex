...
Distill only & & \checkmark & \checkmark & .618$\pm$.005 & .039$\pm$.016 & .888 & 10.8 \\
\midrule
...
Key findings:
\begin{itemize}
  \item \textbf{Without target labels}: Neither replay alone (.563 macro, \textit{worse} than static) nor distillation alone (.618) can repair collapse. Source-domain signals cannot correct target-domain confusion without target supervision. This establishes that \textit{target labels are the essential ingredient}.
