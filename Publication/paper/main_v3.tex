...
% NOTE: fig_ablation_bar.pdf must be regenerated to match the 7-row v3 ablation table.
% Run: python scripts/make_paper_figures.py --output-dir ../../Publication/figures
\begin{figure}[t]
\centering
...
\begin{table}[t]
\centering
\caption{Multi-period results (margin, $B$=1000, 3 seeds). M10 and M12 use all-class replay; M7/M9/M11 use targeted replay (run before the all-class finding). Collapse F1 uses period-specific auto-discovered class sets except M10/M12 which use the fixed 12-class set. Macro-F1 is comparable across all rows (all 178 classes). \ours{}'s benefit increases with drift severity.}
...
Period & Macro & Collapse$^\dagger$ & Macro & Collapse$^\dagger$ & $\Delta$Macro \\
\midrule
...
M12 (severe) & .629 & .028 & .686$\pm$.001 & .256 & +.057 \\
\bottomrule
\multicolumn{6}{l}{\footnotesize $^\dagger$Collapse eval sets differ by period; only $\Delta$Macro is directly comparable.}
\end{tabular}
\end{table}
