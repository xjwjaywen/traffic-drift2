...
We evaluate on CESNET-QUIC22~\cite{luxemburk2023quic} (102 classes, QUIC protocol, weekly granularity, 3-week test span) to test cross-protocol generalization.
Concept drift is milder: the static model maintains macro-F1 of 0.719 with fewer severely collapsed classes (3 classes below 0.1 recall vs.\ 12 on TLS22).
Under strict evaluation (margin, $B{=}1{,}000$, all-class replay $k{=}5$, 5 seeds), \ours{} achieves macro-F1 of $0.718{\pm}0.003$ and collapse-class F1 improves from 0.305 to $0.413{\pm}0.021$.
The modest macro-F1 change ($-$0.001) confirms that \ours{} does no harm when drift is mild, while still recovering collapse classes (+35\% relative collapse F1).
This validates the framework's generalization across encryption protocols and drift regimes.
