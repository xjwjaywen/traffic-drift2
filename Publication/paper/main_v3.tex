...
\begin{table}[t]
\centering
\caption{Component ablation at M-2022-12 (margin, $B$=1000, strict eval, 5 seeds). Each row adds one component to isolate its independent contribution.}
\label{tab:ablation}
\small
\begin{tabular}{@{}l cccc@{}}
\toprule
...
FT + Distill            & .670$\pm$.000 & .152$\pm$.007 & .887 & --- \\
\textbf{Full \ours{}}   & \textbf{.686$\pm$.001} & \textbf{.256$\pm$.009} & .892 & \textbf{5.8} \\
\bottomrule
\end{tabular}
\end{table}
...
Key findings from each component's independent contribution:
\begin{itemize}
  \item \textbf{FT only} (labels alone): Repairs collapse classes (F1: .028$\to$.134) but severely damages stable classes (.903$\to$.839), demonstrating that targeted labels alone cause catastrophic forgetting.
  \item \textbf{FT + Replay} (labels + replay, no distillation): Further improves collapse repair (.134$\to$.223) but does not recover stable classes (.839). Replay alone is insufficient to anchor 178-class decision boundaries. Macro-F1 actually drops below FT-only (.620 vs.\ .638) because replay samples dilute the target signal without the regularization that distillation provides.
  \item \textbf{FT + Distill} (labels + distillation, no replay): Achieves the highest macro-F1 among partial configurations (.670) and recovers stable-class F1 to .887, demonstrating that distillation alone provides strong regularization. However, collapse repair is lower than FT+Replay (.152 vs.\ .223) because without replay the model lacks explicit multi-class anchoring.
  \item \textbf{Full \ours{}} (all three): The best overall trade-off---highest macro-F1 (.686), highest collapse F1 (.256), and near-full stable-class recovery (.892). Replay and distillation are complementary: replay provides the multi-class feature anchors that distillation needs to enforce soft-label consistency.
\end{itemize}
...
\begin{table}[t]
\centering
\caption{Multi-period results (margin, targeted replay, $B$=1000, 3 seeds). \ours{}'s benefit increases with drift severity.}
...
\ours{} is effective at all drift levels, with increasing benefit as drift severity grows (M7: +0.010; M12: +0.045).
