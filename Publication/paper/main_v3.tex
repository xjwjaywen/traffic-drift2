...
A sensitivity analysis (\S\ref{sec:discussion}) shows that under $\pm$50\% perturbations and random Dirichlet sampling, recall remains $\geq$0.71.
...
\begin{itemize}
  ...
  \item \textbf{Triggering}: Per-class risk scores inform \textit{when} repair should be considered, without requiring labeled data to detect drift.
\end{itemize}
...
Recall is stable across all periods (0.857--0.909), confirming that the detection module flags most collapsed classes from M7 onward.
...
The detection module's primary deployment value is its high recall ($\geq$0.86), reducing the risk of missed collapse events.
...
Under $\pm$50\% perturbation, F1 ranges from 0.647 to 0.706 (std = 0.029) with recall 0.786--0.857.
Under Dirichlet-random vectors, the minimum recall is 0.71.
The default weights achieve the highest F1 but are not critical: across all 30 tested configurations, recall remains $\geq$0.71.
