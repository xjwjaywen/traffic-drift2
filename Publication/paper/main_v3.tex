...
\begin{enumerate}
  ...
  \item \textbf{All-class source replay.} $k{=}5$ exemplars per class (890 samples total) from the original training data are mixed into fine-tuning to prevent catastrophic forgetting.
...
\ours{} generalizes across architectures (CNN and Transformer), drift severity (M7--M12), datasets (CESNET-QUIC22), and supports exemplar-free deployment via prototype replay.
...
Our primary classifier uses GroupNorm for stability under non-i.i.d.\ traffic batches.
...
Figure~\ref{fig:tsne} visualizes both mechanisms via t-SNE on encoder features for four representative absorber--victim pairs.
...
These are traffic-adapted implementations for 1D packet-sequence inputs, with necessary simplifications: EATA uses uniform Fisher importance, CoTTA uses Gaussian noise augmentation, and NOTE uses a simplified prediction-balanced buffer.
...
We fine-tune only the classification head $h_\theta$ using the selected target labels combined with $k{=}5$ samples per class from the training data (all-class source replay, 890 samples total):
\begin{equation}
...
All-class replay prevents the head from overfitting to the small target sample and maintains decision boundaries for all 178 classes---a critical advantage over targeted replay, as confirmed by the replay strategy ablation (\S\ref{sec:replay_ablation}).
...
\textbf{Datasets.}
We use CESNET-TLS-Year22~\cite{hynek2024cesnet} (178 classes, 12 months, TLS traffic) as our primary benchmark and CESNET-QUIC22~\cite{luxemburk2023quic} (102 classes, 4 weeks, QUIC traffic) for generalization.
...
\begin{table}[t]
\centering
\caption{Main results on CESNET-TLS-Year22 at M-2022-12. TTA methods use no labels; all other methods use 1{,}000 labels with all-class replay + distillation. Strict evaluation, 5 seeds.}
...
NOTE            & .630 & .027 & .902 & 12 \\
\midrule
\multicolumn{5}{l}{\textit{Active learning baselines (targeted replay, $B$=1000):}} \\
Entropy         & .600$\pm$.004 & .073$\pm$.013 & --- & 9.8 \\
Coreset         & .618$\pm$.008 & .108$\pm$.037 & --- & 9.2 \\
Random          & .656$\pm$.006 & .183$\pm$.031 & --- & 8.0 \\
\midrule
\multicolumn{5}{l}{\textit{\ours{} (all-class replay, $B$=1000, 5 seeds):}} \\
...
\begin{itemize}
  ...
  \item Standard active learning is counterproductive: entropy (0.600) and coreset (0.618) perform \textit{worse} than the static baseline (0.629) because they select globally uncertain samples dominated by high-traffic classes, systematically avoiding the collapse boundary.
...
\begin{table}[t]
\centering
\caption{Component ablation at M-2022-12 (strict eval, 5 seeds). Top block: without target labels (no repair signal). Bottom block: with 1{,}000 target labels (margin selection). Each row isolates one or two components.}
\label{tab:ablation}
\small
...
\cmidrule(lr){2-4}
Configuration & Labels & Replay & Distill & Macro-F1 & Collapse F1 & Stable F1 & Collapsed \\
\midrule
Static (no repair) & & & & .629 & .028 & \textbf{.903} & 12 \\
Replay only & & \checkmark & & .563$\pm$.007 & .044$\pm$.019 & .816 & 10.2 \\
Replay + Distill & & \checkmark & \checkmark & .618$\pm$.005 & .039$\pm$.016 & .888 & 10.8 \\
\midrule
FT only & \checkmark & & & .638$\pm$.000 & .134$\pm$.001 & .839 & 7.0 \\
FT + Replay & \checkmark & \checkmark & & .620$\pm$.003 & .223$\pm$.001 & .839 & 5.0 \\
FT + Distill & \checkmark & & \checkmark & .670$\pm$.000 & .152$\pm$.007 & .887 & 7.8 \\
...
Key findings:
\begin{itemize}
  \item \textbf{Without target labels}: Neither replay alone (.563 macro, \textit{worse} than static) nor replay+distillation (.618) can repair collapse. Source-domain signals cannot correct target-domain confusion without target supervision. This establishes that \textit{target labels are the essential ingredient}.
  \item \textbf{FT only} (labels alone): Repairs collapse classes (.028$\to$.134) but severely damages stable classes (.903$\to$.839)---catastrophic forgetting.
  \item \textbf{FT + Replay}: Improves collapse repair (.134$\to$.223) but does not recover stable classes (.839). Macro-F1 drops below FT-only (.620 vs.\ .638) because replay dilutes the target signal without regularization.
  \item \textbf{FT + Distill}: Achieves .670 macro and recovers stable F1 to .887, demonstrating strong regularization from distillation. However, collapse repair is lower than FT+Replay (.152 vs.\ .223).
  \item \textbf{Full \ours{}}: The only configuration achieving the best trade-off---highest macro-F1 (.686), highest collapse F1 (.256), and near-full stable recovery (.892). Replay and distillation are complementary: replay provides multi-class feature anchors, distillation enforces soft-label consistency.
\end{itemize}
...
Table~\ref{tab:multiperiod} shows \ours{}'s effectiveness across five time points with increasing drift severity.

\begin{table}[t]
\centering
\caption{Multi-period results (margin, $B$=1000). M10 uses all-class replay (3 seeds); M7/M9/M11/M12 use targeted replay (3 seeds). \ours{}'s benefit increases with drift severity.}
...
M10          & .684 & .258 & .726$\pm$.001 & .396 & +.043 \\
M11 (heavy)  & .649 & .017 & .689$\pm$.002 & .277 & +.040 \\
M12 (severe) & .629 & .031 & .686$\pm$.001 & .256 & +.057 \\
\bottomrule
\end{tabular}
\end{table}

\ours{} is effective at all drift levels, with increasing benefit as drift severity grows (M7: +0.010; M12: +0.057).
Collapse repair is most successful at M7, where the absorber--victim separation in feature space is still partially intact (collapse F1 = 0.456 vs.\ 0.238 at M12).
...
Table~\ref{tab:detect_signals} shows how each signal contributes to detection accuracy at M12 (14 ground-truth collapsed classes).
...
\subsection{Extension: BatchNorm Architecture Verification}
\label{sec:bn_verify}
...
\subsection{Extension: Transformer Architecture}
...
\subsection{Extension: Generalization to QUIC22}
\label{sec:quic22}

On CESNET-QUIC22, concept drift is milder: the static model maintains macro-F1 of 0.719 with fewer severely collapsed classes.
Under strict evaluation (margin, 5 seeds), \ours{} achieves macro-F1 of $0.718{\pm}0.003$---essentially unchanged from Static.
Collapse-class F1 improves modestly (0.305$\to$0.413), confirming that \ours{}'s benefit scales with collapse severity.
When drift is mild, the method does no harm but provides limited gain---expected behavior for a collapse-targeted framework.

\subsection{Extension: Prototype-Based Replay}
...
\ours{} requires 1{,}000 target labels per repair cycle ($<$0.3\% of the ${\sim}$330K monthly test set) plus 890 all-class replay samples from existing training data.
The replay incurs zero additional labeling effort---it reuses the original training data directly.
...
(3)~all-class replay from training data is the default, with prototype replay as a privacy-preserving alternative;
...
On QUIC22, where drift is mild, macro-F1 improvement is negligible; \ours{} is designed for collapse-heavy regimes.
...
\textbf{Training seed audit.}
To verify that absorber-collapse is a property of the data rather than a training artifact, we conduct a deterministic retraining audit: 3 training seeds $\times$ 3 \ours{} seeds = 9 runs yield macro-F1 of $0.667{\pm}0.007$ and collapse F1 of $0.290{\pm}0.034$.
...
A clean component ablation confirms that fine-tuning, replay, and distillation each contribute independently: FT alone reaches 0.638 macro-F1, adding distillation lifts it to 0.670, and the full combination achieves 0.686.
...
The framework generalizes across drift severity (M7--M12), architectures (CNN/Transformer), datasets (QUIC22), and training seeds.
