...
An unsupervised detection module provides a deployment trigger by monitoring per-class health without labels.
...
\begin{enumerate}
  ...
  \item \textbf{Label-efficient repair framework.} We propose \ours{}, combining pair-agnostic margin/BADGE selection, all-class replay, head-only fine-tuning, and knowledge distillation. An unsupervised detection module (recall $\geq 0.86$, F1 0.71) provides a deployment trigger and diagnostic monitoring (\S\ref{sec:method}, \S\ref{sec:experiments}).
\end{enumerate}
...
A sensitivity analysis (\S\ref{sec:discussion}) confirms that any reasonable weighting emphasizing count drop and FD achieves recall $\geq$0.71.
...
Table~\ref{tab:trigger} evaluates detection consistency: recall is $\geq$0.86 at all time points.
...
The detection module's primary deployment value is its high recall ($\geq$0.86), ensuring that collapse is detected whenever it occurs.
...
(1)~the detection module monitors class health and provides a repair trigger without labels;
...
An unsupervised detection module monitors per-class health and provides a deployment trigger when drift is detected; experiments evaluate repair conditional on this trigger.
