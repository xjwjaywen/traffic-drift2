...
\begin{equation}
  s_c = w_1 \cdot \max(0, 1{-}r_c) + w_2 \cdot \tilde{d}_c + w_3 \cdot \tilde{m}_c + w_4 \cdot \tilde{f}_c
\end{equation}
where $\tilde{\cdot}$ denotes MAD-normalized values and $w = (0.4, 0.2, 0.15, 0.15)$ by default.
...
\begin{table}[t]
\centering
\caption{Labeled probe vs.\ autonomous pipeline (margin, $B$=1000, M12, 3 seeds).}
\label{tab:e2e}
\small
\begin{tabular}{@{}l ccc@{}}
\toprule
Pipeline & Macro-F1 & Collapse F1 & Probe labels? \\
\midrule
Static & .629 & .028 & --- \\
Labeled probe + CARE & .673$\pm$.001 & .233$\pm$.002 & Yes \\
\textbf{Autonomous + CARE} & \textbf{.674$\pm$.002} & \textbf{.278$\pm$.004} & \textbf{No} \\
\bottomrule
\end{tabular}
\end{table}

The autonomous pipeline matches or exceeds the labeled-probe version (collapse F1: 0.278 vs.\ 0.233) because unsupervised detection identifies a broader set of at-risk classes (20 candidates vs.\ 12 fixed), giving CARE's replay component better coverage. This eliminates the need for any labeled probe period---the only labeling cost is the 1{,}000 target samples selected for repair.
...
Under strict evaluation (5 seeds), \ours{} improves collapse-class F1 from 0.028 to $0.278{\pm}0.004$ and macro-F1 from 0.629 to $0.674{\pm}0.002$, with consistent gains across drift severity (M7--M12), architectures (CNN/Transformer), and an exemplar-free deployment mode.
