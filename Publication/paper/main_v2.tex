...
To confirm this failure is not caused by our use of GroupNorm, we train an identical CNN with BatchNorm and re-evaluate all TTA methods.
On the BN model at M12, BN-Adapt now functions (macro-F1: 0.622 vs.\ Static 0.613) but collapse F1 remains at 0.032---virtually unchanged from Static (0.030).
Tent (0.027), SAR (0.026), and EATA (0.028) are similarly ineffective; NOTE catastrophically degrades performance (macro-F1: 0.042).
...
\textbf{Metrics.}
Overall macro-F1 (averaged over all 178 classes), collapse-class macro-F1 (averaged over the 12 classes with recall $<0.1$ in the static model at M12), stable-class macro-F1 (averaged over 20 classes with consistently high recall across all periods; the overall macro-F1 implicitly covers all remaining classes), and the number of collapsed classes (recall $< 0.1$).
...
To verify that absorber-collapse is a property of the data rather than a training artifact, we train three models with different random seeds. All three produce 11--12 collapsed classes at M12, with macro-F1 ranging from 0.620 to 0.629 and collapse F1 from 0.028 to 0.036. The collapse class sets overlap substantially, confirming that the absorber-collapse pattern is an inherent property of the dataset's temporal drift, not an artifact of a particular training run. The 5-seed standard deviations reported in our main results reflect selection and fine-tuning randomness only; source-training variance is small by comparison.
