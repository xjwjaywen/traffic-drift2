...
\textbf{Autonomous (3s)} & \textbf{.673$\pm$.000} & \textbf{.231$\pm$.002} & \textbf{5.0} & \textbf{No} \\
\bottomrule
\end{tabular}
\end{table}

When evaluated on the same 12-class set, the autonomous pipeline achieves virtually identical performance to the labeled-probe version (collapse F1: 0.231 vs.\ 0.233, macro-F1: 0.673 vs.\ 0.673).
...
To verify that absorber-collapse is a property of the data rather than a training artifact, we train three models with different random seeds (with deterministic cuDNN settings) and run the full CARE pipeline on each.
All three produce 11--12 collapsed classes at M12, with static macro-F1 ranging from 0.620 to 0.621 and collapse F1 from 0.030 to 0.036---a training-seed standard deviation of only $\pm$0.0006 for macro-F1.
Applying CARE (margin, $B{=}1{,}000$) to each training seed yields macro-F1 of $0.667{\pm}0.007$ and collapse F1 of $0.290{\pm}0.034$ across training seeds ($3\times3$ = 9 runs), confirming that the improvement is robust to model initialization.
Training-seed variance for the static baseline ($\sigma{=}0.0006$) is an order of magnitude smaller than fine-tuning-seed variance ($\sigma{\approx}0.001$), and both are small relative to the CARE improvement ($+$0.044 macro-F1). This confirms that the absorber-collapse pattern is an inherent property of the dataset's temporal drift, not an artifact of a particular training run.
