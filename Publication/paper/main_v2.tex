...
We propose \ours{} (\textbf{C}ollapse-\textbf{A}ware \textbf{RE}pair), a targeted maintenance framework that (1)~discovers absorber-victim class pairs from a small labeled probe set in a preceding time period, (2)~selects informative samples via margin-based active selection, and (3)~fine-tunes only the classification head using the selected labels combined with source-domain replay and knowledge distillation.
...
Experiments on CESNET-TLS-Year22 (178 classes, 12 months) and CESNET-QUIC22 (102 classes, 4 weeks) with strict evaluation (queried samples excluded from test set, 5 random seeds) demonstrate that \ours{} improves collapse-class F1 from $0.028$ to $0.233{\pm}0.002$ ($8\times$) and overall macro-F1 from $0.629$ to $0.673{\pm}0.001$ using 1{,}000 labeled target samples ($<$0.3\% of the test set) plus a small labeled probe set from a preceding period, without requiring an oracle list of affected classes.
The \ours{} framework is effective across time points (M7--M12), architectures (CNN and Transformer), and selection strategies (margin for stability, BADGE for maximum recovery). It further supports exemplar-free deployment via prototype-based replay with $<$1\% performance loss.
\end{abstract}
...
\begin{enumerate}
  ...
  \item We propose the \ours{} framework for collapse repair, combining active selection, source replay, and knowledge distillation. We evaluate multiple selection strategies, recommending \ours{}-Margin as the deployable default (lowest variance, sub-second compute) while showing that BADGE~\cite{ash2020badge} achieves higher collapse recovery at the cost of stability. We further demonstrate prototype-based replay for exemplar-free deployment and a confidence-based trigger mechanism (\S\ref{sec:method}, \S\ref{sec:experiments}).
\end{enumerate}
...
The complete pipeline---probe-based absorber discovery, confidence-based trigger, and targeted repair---requires only a small labeled probe set and 1{,}000 target labels per repair cycle, making \ours{} practical for real-world encrypted traffic monitoring.
