...
Experiments on CESNET-TLS-Year22 (178 classes, 12 months) and CESNET-QUIC22 (102 classes, 4 weeks) demonstrate that \ours{} recovers 7 out of 12 collapsed classes, improving collapse-class F1 from 0.028 to 0.262 (9$\times$) while maintaining stable-class performance (F1: 0.903$\to$0.895). With only 1{,}000 labeled target samples ($<$0.6\% of the test set) and no prior knowledge of which classes are affected, \ours{} achieves an overall macro-F1 of 0.686, outperforming the best baseline by 5.8 percentage points. When absorber identity is available, collapse-class F1 further improves to 0.426 (15$\times$).
\end{abstract}
...
BN-Adapt$^a$    & 0.629 & 0.028 & 0.903 & 12 \\
Tent        & 0.631 & 0.026 & 0.903 & 12 \\
EATA        & 0.629 & 0.028 & 0.903 & 12 \\
CoTTA       & 0.629 & 0.027 & 0.903 & 12 \\
SAR         & 0.629 & 0.026 & 0.901 & 11 \\
NOTE$^a$        & 0.630 & 0.027 & 0.902 & 12 \\
\midrule
\multicolumn{5}{l}{\textit{Methods using 1{,}000 labeled target samples:}} \\
Naive-Head$^b$  & 0.631 & 0.034 & 0.890 & 11 \\
Naive-Full$^b$  & 0.614 & 0.203 & 0.856 & 8 \\
\textbf{\ours{}}$^b$ & \textbf{0.686} & \textbf{0.262} & 0.895 & \textbf{5} \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^a$Non-functional on GroupNorm architecture (see \S\ref{sec:related}).}\\
\multicolumn{5}{l}{\footnotesize $^b$1{,}000 random labeled target samples. \ours{} adds 890 replay samples.}\\
\multicolumn{5}{l}{\footnotesize Naive-Head = head-only FT. Naive-Full = full-model FT. Both: no replay/distill.}
\end{tabular}
\end{table}
...
On CESNET-TLS-Year22, \ours{} recovers 7 of 12 collapsed classes and improves overall macro-F1 by 5.8\% with only 1{,}000 labeled target samples.
