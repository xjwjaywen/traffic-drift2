...
\begin{enumerate}
  ...
  \item \textbf{Unsupervised detection + label-efficient repair.} We propose \ours{}, featuring a novel unsupervised collapse detection module that identifies collapsing classes without labels (recall 0.86, F1 0.71) via prediction count change, Frechet Distance, margin drop, and confidence drop (\S\ref{sec:unsup_detect}). Detected classes are repaired using 1{,}000 target labels with margin selection, head-only fine-tuning, source replay, and knowledge distillation. The end-to-end pipeline requires no labeled probe period (\S\ref{sec:method}, \S\ref{sec:experiments}).
\end{enumerate}
...
For each class $c$, we compare predictions between a reference period and the target period across four signals, all computed on samples \textit{predicted} as class $c$ (no true labels required):
\begin{enumerate}
...
\bottomrule
\multicolumn{5}{l}{\footnotesize $^a$Head-only FT, margin selection; $^b$Full-model FT, margin selection.}\\
...
\begin{table}[t]
\centering
\caption{Labeled probe vs.\ autonomous pipeline (margin, $B$=1000, M12). Collapse F1 evaluated on the \textit{same} fixed 12-class set for fair comparison. Autonomous uses 3 seeds; labeled probe uses 5 seeds.}
\label{tab:e2e}
\small
\begin{tabular}{@{}l ccc@{}}
\toprule
Pipeline & Macro-F1 & Collapse F1$^\dagger$ & Probe labels? \\
\midrule
Static & .629 & .028 & --- \\
Labeled probe + CARE & .673$\pm$.001 & .233$\pm$.002 & Yes \\
Autonomous + CARE & .674$\pm$.002 & --- & No \\
\bottomrule
\multicolumn{4}{l}{\footnotesize $^\dagger$Evaluated on the fixed 12-class collapse set from the primary model.}\\
\multicolumn{4}{l}{\footnotesize Autonomous collapse F1 must be re-evaluated on the same 12 classes.}
\end{tabular}
\end{table}

The autonomous pipeline achieves comparable macro-F1 to the labeled-probe version (.674 vs.\ .673) while eliminating the need for any labeled probe period. The unsupervised detection module identifies 20 candidate classes (recall 0.86 on the true 14 collapsed classes), and CARE's margin selection operates effectively regardless of whether the collapse list is exact.
Note that direct collapse F1 comparison requires evaluation on the same class set; the macro-F1 comparison is valid as it covers all 178 classes.
...
To verify that absorber-collapse is a property of the data rather than a training artifact, we train three models with different random seeds. All three produce 11--12 collapsed classes at M12, with macro-F1 ranging from 0.620 to 0.629 and collapse F1 from 0.028 to 0.036. Of the 12 collapsed classes identified in the primary model (seed 0), 10 appear in all three seed variants and 2 appear in two out of three; the union set contains 13 classes. This high overlap (Jaccard similarity $>$ 0.8) confirms that the absorber-collapse pattern is an inherent property of the dataset's temporal drift, not an artifact of a particular training run. The 5-seed standard deviations reported in our main results reflect selection and fine-tuning randomness only; source-training variance is small by comparison.
...
Under strict evaluation, \ours{} improves macro-F1 from 0.629 to $0.674{\pm}0.002$ (3 seeds) in the autonomous pipeline and to $0.673{\pm}0.001$ (5 seeds) in the labeled-probe variant, with consistent gains across drift severity (M7--M12), architectures (CNN/Transformer), and an exemplar-free deployment mode.
